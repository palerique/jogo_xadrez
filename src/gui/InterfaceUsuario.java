...
public class InterfaceUsuario  {
	...
	public static PosicaoXadrez readChessPosition(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro ao ler PosicaoXadrez. Valores válidos são de a1 até h8.");
		}
	}
	...
	private static void printCapturedPieces(List<PecaXadrez> captured) {
		List<PecaXadrez> white = captured.stream().filter(x -> x.getColor() == Cor.WHITE).collect(Collectors.toList());
		List<PecaXadrez> black = captured.stream().filter(x -> x.getColor() == Cor.BLACK).collect(Collectors.toList());
